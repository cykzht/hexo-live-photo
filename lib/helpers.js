...
module.exports.injectAssets = function(hexo, config) {
  ...
  const configScript = `
    <script>
      window.LivePhotoConfig = ${JSON.stringify(config)};
    </script>
  `;
  
  // 注入到head中
  injector.register('head_begin', () => configScript);
  injector.register('head_begin', () => `<style>${css}</style>`);
  injector.register('body_end', () => `<script>${js}</script>`);
};